...
US <- read_csv("/Users/krisztafarkas/Box Sync/Google-search-data/2019ExploratoryData/IPV/IPV-hit-1/states/US/US-IPV-hit-1.csv")

for(i in state_list[1:51]){
  counter <- 1
  assign(i, read_csv(paste0("/Users/krisztafarkas/Box Sync/Google-search-data/2019ExploratoryData/IPV/IPV-hit-1/states/", i, "/", i, "-IPV-hit-1.csv")
  )
)
}
...
p1a <- ggplot(northeast, aes(x = timestamp, y = value)) + geom_line(aes(col = state)) + 
  ylim(0,125) +
  labs(x = "Date", y = "Search outcome - 'hit'") + 
  ggtitle("\n\n\n'Hit' - Northeastern States, with total US (black line)") +
...
p2a <- ggplot(mdwest, aes(x = timestamp, y = value)) + geom_line(aes(col = state)) +
  ylim(0,125) +
  labs(x = "Date", y = "Search outcome - 'hit'") + 
  ggtitle("\n\n\n'Hit' - Midwestern States, with total US (black line)") +
  geom_line(data = all %>% filter(state == "Total US"), linetype = "longdash") +
  geom_dl(aes(label = state), method = list(dl.combine("first.points", "last.points"), cex = 0.8)) 
...
p3a <- ggplot(south, aes(x = timestamp, y = value)) + geom_line(aes(col = state)) + 
  ylim(0,125) +
  labs(x = "Date", y = "Search outcome - 'hit'") + 
  ggtitle("\n\n'Hit' - Southern States, with total US (black line)") +
  geom_line(data = all %>% filter(state == "Total US"), linetype = "longdash") +
  geom_dl(aes(label = state), method = list(dl.combine("first.points", "last.points"), cex = 0.8)) 
...
p4a <- ggplot(west, aes(x = timestamp, y = value)) + geom_line(aes(col = state)) + 
  ylim(0,125) +
  labs(x = "Date", y = "Search outcome - 'hit'") + 
  ggtitle("\n\n'Hit' - Western States, with total US (black line)") + 
  geom_line(data = all %>% filter(state == "Total US"), linetype = "longdash") +
  geom_dl(aes(label = state), method = list(dl.combine("first.points", "last.points"), cex = 0.8)) 
